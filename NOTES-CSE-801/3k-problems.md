...
- **Additive Increase:** The _difference_ `R1 - R2` remains constant. The _ratio_ `R1/R2` changes.
- **Multiplicative Decrease:** The _ratio_ `R1/R2` remains constant.
...
3.  **Simultaneous Loss:** The buffer becomes completely full. The next packets to arrive from _several different flows_ are all dropped at roughly the same time.
...
- **Example: Random Early Detection (RED):** Instead of waiting for the queue to be completely full, a RED-enabled router starts dropping packets _probabilistically_ as the average queue size grows.
...
- **Cons:**
  - **Extremely Dangerous:** If the available bandwidth has _decreased_ while the connection was idle, resuming with a large `cwnd` will instantly flood the network, causing a massive burst of packet loss. This is highly disruptive to the connection itself and to other flows on the network.
